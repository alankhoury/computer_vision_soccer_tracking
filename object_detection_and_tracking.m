%remove the audience form the frame using kmeans clustering and graph cut
% first extract the payers from a frame and output the position of the
% players in pixel frame.

% once the position of the players are found in this first frame, use
% rudimentary player tracking by repeating this process for the next frame
% and comparing the distance the blobs have moved and match the objects
% together
...
% run through each frame and for each, 
% 1 use graphcut to remove audience
% 2 detect players
% compare previous frame and player locations and use icp to update player
% location in pixel frame

trackPlayers(rawVidObj, startFrame, endFrame, 1);
...
%% 
function trackPlayers(rawVidObj, startFrame, endFrame, frameStep)

for frameCount = startFrame:frameStep:endFrame
    %remove audience
    removeCrowd()
    % detect players
