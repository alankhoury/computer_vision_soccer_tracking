...
img = im2double( imresize(imread([ImageDir 'fifa-shot-1.png']), 0.125) );
...
imgd(:,1) = imgd(:,1)/max(imgd(:,1));   imgd(:,2) = imgd(:,2)/max(imgd(:,2));


% We ask the k-means algorithm for four clusters
...
figure, imagesc(kmlabels) ; axis image ;  axis off;
...
label=gclabels(100,200) ; %look for the label coordinate corresponding to the main class (e.g. the rhino)
lb=(gclabels==label) ;
